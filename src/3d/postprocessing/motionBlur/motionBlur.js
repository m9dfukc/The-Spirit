...
import motionBlurLinesFragmentShader from "./motionBlurLines.frag";
import motionBlurLinesVertexShader from "./motionBlurLines.vert";
import motionBlurSamplingFragmentShader from "./motionBlurSampling.frag";
...
function init(sampleCount) {
  ...
  _linesMaterial = new THREE.RawShaderMaterial({
    ...
    vertexShader: fboHelper.rawShaderPrefix + motionBlurLinesVertexShader,
    fragmentShader: fboHelper.rawShaderPrefix + motionBlurLinesFragmentShader,
  ...
  _samplingMaterial = new THREE.RawShaderMaterial({
    ...
    fragmentShader:
      ...
      (sampleCount || 21) +
      "\n" +
      motionBlurSamplingFragmentShader,
  });
}
...
function render(dt, renderTarget, toScreen) {
  ...
  let useSampling = exports.useSampling;
  let fpsRatio = 1000 / (dt < 16.667 ? 16.667 : dt) / exports.targetFPS;
  ...
  for (let i = 0, len = _visibleCache.length; i < len; i++) {
